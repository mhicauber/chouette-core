class Export::GTFS < Export::Base
  ...
  def export_time_tables_to(target)
    ...
    Chouette::TimeTable.including_vehicle_journeys_within_@date_range(vehicule_journey_ids, @date_range).each do |time_table|
      time_table_dates = timetable.time_table_dates
  ...
  def export_vehicle_journey_at_stops_to(target)
    @journeys.each do |vehicle_journey|
      vehicle_journey.vehicle_journey_at_stops.each do |vehicle_journey_at_stop|
        @vehicule_journey_service_trip_hash[vehicle_journey].each do |trip_id|
