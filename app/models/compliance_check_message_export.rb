  ...
  def to_csv(options = {})
    csv_string = CSV.generate(options.slice(:col_sep, :quote_char, :force_quotes)) do |csv|
      ...
      compliance_check_messages.each do |compliance_check_message|
        puts compliance_check_message.message_key
        puts I18n.t("compliance_check_messages.#{compliance_check_message.message_key}", compliance_check_message.message_attributes.deep_symbolize_keys)
