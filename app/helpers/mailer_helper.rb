module MailerHelper
  def mailer_link_to text, url, opts={}
    link_to text, url, opts.update(style: mailer_style(:link))
  end
  ...
  def mailer_style component
    ...
    if File.exists? custom_stylesheet_path
  ...
  def mailer_base_css
    ...
    sass_engine = Sass::Engine.new stylesheet, load_paths: [mailer_stylesheets_path]
    "<style>#{sass_engine.render}</style>".html_safe
  end

  def render_custom name
    if !Dir.glob(File.join(Rails.root, "app", "views", "layouts", "mailer", "custom", "_#{name}.html.*")).empty?
